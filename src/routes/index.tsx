    ...
    <div className="flex flex-col">
      <section className="relative hidden min-h-dvh md:block">
        ...
        <div className="relative flex min-h-dvh flex-col justify-end px-10 pb-12 pt-28">
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            <Link
              to="/hotele"
              className="btn-cyan inline-flex h-12 items-center rounded-full px-6 text-sm font-semibold"
            >
              Rezerwuj nocleg
            </Link>
            <Link
              to="/nieruchomosci"
              className="inline-flex h-12 items-center rounded-full border border-white/25 bg-white/10 px-6 text-sm text-white backdrop-blur-lg"
            >
              Zatrudnij eksperta
            </Link>
          </div>
          ...
          <div className="mt-10 grid grid-cols-4 gap-3">
            {tiles.map((s) => (
              <Link
                key={s.slug}
                to={s.href}
                className="flex flex-col space-y-2 rounded-2xl border border-white/20 bg-white/10 p-4 backdrop-blur-xl transition hover:border-cyan-400/50 hover:bg-white/15"
              >
      ...
      <section className="relative md:hidden">
        ...
        <div className="absolute inset-x-0 bottom-0 flex flex-col space-y-1 px-5 pb-5">
          ...
          <h1 className="font-display text-[2.35rem] leading-none text-white">
            {SITE.name}
          </h1>
      ...
      <section className="flex flex-col space-y-3 px-5 pb-6 pt-5 md:hidden">
        <h2 className="text-[11px] font-medium uppercase tracking-[0.22em] text-gray-300">
          Usługi
        </h2>
        ...
        <Link
          to="/wspolpraca"
          className="flex flex-col space-y-1 rounded-[1.4rem] border border-white/20 bg-white/10 px-4 py-4 backdrop-blur-lg"
        >
          <span className="text-[11px] font-medium uppercase tracking-[0.22em] text-gray-300">
            Imate nekretninu?
          </span>
          <span className="text-base font-medium text-white">
            Želite da prodate? Ostavite kontakt.
          </span>
          <span className="text-sm text-gray-300">
            Javićemo se. Unos radimo mi.
          </span>
        </Link>
      </section>
    </div>
